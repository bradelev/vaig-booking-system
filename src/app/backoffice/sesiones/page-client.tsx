...
import SessionsDayTable, {
  type Professional,
  type SessionRow,
  type PendingBookingData,
} from "@/components/backoffice/sesiones/sessions-day-table";
...
export default function SessionsPageClient({
  ...
  function goToPrevWeek() {
    router.push(`/backoffice/sesiones?semana=${addDays(weekMonday, -7)}`);
  }
  ...
  function goToCurrentWeek() {
    router.push(`/backoffice/sesiones`);
  }
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 p-4 sm:p-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold text-foreground">Sesiones</h1>
          <p className="text-sm text-muted-foreground mt-1">Registro diario y resumen</p>
        </div>
        ...
        >
          ...
          {syncResult && (
            <div className="space-y-1">
              <p>
                Sync completado — <strong>{syncResult.imported}</strong> insertados,{" "}
                <strong>{syncResult.skipped}</strong> omitidos
              ...
              {syncResult.errors.length > 0 && (
                <details className="mt-1">
                  <summary className="cursor-pointer text-destructive text-xs">Ver errores</summary>
      ...
      {/* Sessions card — table + inline creation + footer */}
      <div className="rounded-xl border border-border bg-card p-4 sm:p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold text-foreground">
            {DAY_LABELS[activeTab]} {formatDDMM(activeDate)}
            {activeDate === today && (
              <span className="ml-1.5 text-primary font-medium">(hoy)</span>
            )}
          </h2>
        </div>
        <SessionsDayTable
