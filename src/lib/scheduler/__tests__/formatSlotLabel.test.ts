...
describe("formatSlotLabel", () => {
  it("formats a Monday slot correctly", () => {
    ...
    expect(label.includes("Lunes"), `Expected 'Lunes' in: ${label}`).toBeTruthy();
    // es-AR locale may format day/month without leading zeros (23/3)
    expect(label.includes("23"), `Expected '23' (day) in: ${label}`).toBeTruthy();
    expect(label.includes("3"), `Expected '3' (month) in: ${label}`).toBeTruthy();
    expect(label.includes("10:00"), `Expected '10:00' in: ${label}`).toBeTruthy();
    expect(label.includes("a las"), `Expected 'a las' in: ${label}`).toBeTruthy();
  });
  ...
  it("formats a Tuesday slot correctly", () => {
    ...
    const label = formatSlotLabel(date);
    expect(label.includes("Martes"), `Expected 'Martes' in: ${label}`).toBeTruthy();
    expect(label.includes("24"), `Expected '24' in: ${label}`).toBeTruthy();
    expect(label.includes("14:00"), `Expected '14:00' in: ${label}`).toBeTruthy();
  });
  ...
  it("formats a Saturday slot correctly", () => {
    ...
    const label = formatSlotLabel(date);
    expect(label.includes("Sábado"), `Expected 'Sábado' in: ${label}`).toBeTruthy();
    expect(label.includes("21"), `Expected '21' in: ${label}`).toBeTruthy();
  });
  ...
  it("formats a Sunday slot correctly", () => {
    ...
    const label = formatSlotLabel(date);
    expect(label.includes("Domingo"), `Expected 'Domingo' in: ${label}`).toBeTruthy();
    expect(label.includes("22"), `Expected '22' in: ${label}`).toBeTruthy();
  });
  ...
  it("formats end-of-month date correctly (31/03)", () => {
    ...
    const label = formatSlotLabel(date);
    expect(label.includes("31"), `Expected '31' in: ${label}`).toBeTruthy();
    expect(label.includes("12:00"), `Expected '12:00' in: ${label}`).toBeTruthy();
  });
  ...
  it("formats start-of-month in April correctly", () => {
    ...
    const label = formatSlotLabel(date);
    expect(label.includes("1"), `Expected '1' (day) in: ${label}`).toBeTruthy();
    expect(label.includes("4"), `Expected '4' (April) in: ${label}`).toBeTruthy();
  });
  ...
  it("formats time with minutes correctly (09:30)", () => {
    ...
    const label = formatSlotLabel(date);
    expect(label.includes("09:30"), `Expected '09:30' in: ${label}`).toBeTruthy();
  });
  ...
  it("includes the correct day name for all weekdays", () => {
    ...
    for (const [utc, expectedDay] of cases) {
      ...
      const label = formatSlotLabel(date);
      expect(label.includes(expectedDay), `Expected '${expectedDay}' in: ${label} (UTC: ${utc})`).toBeTruthy();
    }
  });
});
